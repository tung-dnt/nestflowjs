...
description: Build your own adapter by extending BaseWorkflowAdapter to run workflows on any computing service.
---
...
Build your own adapter by extending `BaseWorkflowAdapter` — the abstract class that owns the orchestration loop and dispatches each [TransitResult](/docs/api-reference/adapters#transitresult) to a handler method you implement.
...
- [TransitResult](/docs/api-reference/adapters#transitresult) — understand the result types
- [DurableLambdaEventHandler](/docs/plugins/durable-lambda) — built-in durable adapter
