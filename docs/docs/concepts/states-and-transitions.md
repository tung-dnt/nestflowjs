...
States and transitions form the backbone of every workflow. States represent where an entity is; transitions define the legal moves between them.
...
6. Determine the [TransitResult](../api-reference/adapters#transitresult)
...
- [Workflow Definition](./workflow-definition) — the `@Workflow` decorator and full definition shape
- [Events and Handlers](./events-and-handlers) — `@OnEvent`, `@OnDefault`, parameter injection
- [TransitResult](../api-reference/adapters#transitresult) — what `transit()` returns
