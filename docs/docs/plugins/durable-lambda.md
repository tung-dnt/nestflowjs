...
Long-running workflows that span minutes, hours, or days. Checkpoint each step, pause for human input, and resume exactly where you left off.
...
The adapter loops over `transit()` calls, reacting to each [TransitResult](../api-reference/adapters#transitresult):
...
- [Adapters API](../api-reference/adapters) — TransitResult, BaseWorkflowAdapter
...
- [Lambda Order Example](../examples/lambda-order-state-machine) — complete deployment example
